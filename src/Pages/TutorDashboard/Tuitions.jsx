...
const Tuitions = () => {
    ...
    const handleSubjectChange = (e) => {
        ...
        setSubject(subject);
        if (subject === '' && districtName === '') {
            // If 'All' is selected, show all tuitions
            console.log("All")
            setFilteredTuitions(allTuitions);
        }
        else if (districtName === "" && subject !== "") {
            const filteredData = allTuitions.filter(tu => tu.details.subjects.includes(subject));
            console.log("filteredData", filteredData)
            setFilteredTuitions(filteredData);
        }
        else {
            const filteredData = allTuitions.filter(tu => tu.details.subjects.includes(subject) && tu.district === districtName);
            console.log("filteredData", filteredData)
            setFilteredTuitions(filteredData);
        }
    }
        ...
        <>
            ...
            <div className='flex flex-col md:flex-row lg:flex-row gap-2 w-full z-30 mb-9'>
                ...
                <div className=''>
                    ...
                    <select id="district" name="district_id" onChange={handleSubjectChange} className='w-32 focus:outline-none border rounded p-2 m-2 capitalize round'>
                        <option value="">All</option>
                        {
                            ["bangla", "english", "math", "higher math", "science", "history", "geography", "politics", "biology", "physics", "chemistry", "accounting", "ict"].map((subject, index) => (
                                <option key={index} value={subject} className='capitalize'>{subject}</option>
                            ))
                        }
                    </select>
                </div>
                    ...
                    filteredTuitions.length > 0 ? filteredTuitions.map(tuition =>
                        <div class="tuition-card text-sm w-[90%] md:w-[40vw] lg:w-[24vw] h-[320px] lg:h-[310px]">
                            ...
                            <div className='flex justify-center mt-6'>
                                <button className='btn btn-sm border shadow-2xl rounded-3xl bg-[#29ADB2] hover-[#29ADB2]  hover:bg-gradient-to-t hover:from-[#0766AD] hover:to-[#29ADB2] hover:text-white text-black lg:w-1/2 md:w-1/3 w-[80%] focus:outline-none capitalize font-thin' onClick={() => {
